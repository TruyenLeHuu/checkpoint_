...
$(document).ready(function () {
	...
	function intervalUpdateTime() {
		...
		functionPoint = setInterval(function () {
			...
			document.getElementById("timerCount").innerHTML = minutes + ":" + seconds + ":" + mils;
	...
	function updateCheckpoint(number, team) {
		...
		for (let i = 1; i <= number; i++)
			$('#checkpoint' + i + teamSel).css({ 'background-color': '#7FFF00', 'box-shadow': '0 1px 18px 10px rgba(127, 255, 0, 0.219), 0 6px 20px 0  rgba(127, 255, 0, 0.219)' });
		for (let i = number + 1; i <= 10; i++)
			$('#checkpoint' + i + teamSel).css({ 'background-color': '#bbb', 'box-shadow': '0 0px 0px 0px white, 0 0px 0px 0px white' });
		if (number == 0) {
			for (let i = 1; i <= 10; i++)
				$('#checkpoint' + i + teamSel).css({ 'background-color': '#bbb', 'box-shadow': '0 0px 0px 0px white, 0 0px 0px 0px white' });
		}
